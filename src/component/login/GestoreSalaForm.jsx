...
function GestoreSalaForm() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (isEditing && user && id === reduxUserId) {
        ...
        try {
          const res = await fetch(`${apiUrl}/gestori/${id}`, {
          ...
          console.error("Errore fetch gestore sala prove", err);
        }
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch(`${apiUrl}/${isEditing ? `gestori/${id}` : "register-gestore-sp"}`, {
  ...
  return (
    <div>
      <h1 className="text-center metal-mania-regular">
        {isEditing ? "Modifica Profilo Gestore Sala Prove" : "Registrati come Gestore di Sala Prove"}
      </h1>
