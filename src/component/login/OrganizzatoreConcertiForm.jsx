...
function OrganizzatoreConcertiForm() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (isEditing && user && id === reduxUserId) {
        ...
        try {
          const res = await fetch(`${apiUrl}/organizzatori/${id}`, {
          ...
          console.error("Errore fetch organizzatore", err);
        }
      }
    };
  ...
  }, [id, user, isEditing, token, apiUrl]);
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch(`${apiUrl}/${isEditing ? `organizzatori/${id}` : "register-organizzatore"}`, {
  ...
  return (
    <div>
      <h1 className="text-center metal-mania-regular">
        {isEditing ? "Modifica Profilo Organizzatore" : "Registrati come Organizzatore di Eventi"}
      </h1>
